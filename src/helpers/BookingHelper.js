...
export const canExistBooking = (
  ...
  let numberOfDays =
    newEndDate === undefined
      ? numberOfDaysOfBooking(currentBooking)
      : numberOfDaysBetweenDates(newEndDate, newStartDate);
  ...
  let conflictedBookings = allBookings.filter((filterSingleBooking) => {
    ...
    if (
      filterSingleBooking.room_id === room_id &&
      currentBooking.id !== filterSingleBooking.id
    ) {
      // check if dates matches
      let filterSingleBookingDates = getAllBookingDates(filterSingleBooking);
      let commonDates = filterSingleBookingDates.filter(
        (tempDate) => targetBookingDates.indexOf(tempDate) > -1
      );
    ...
    allBookings = allBookings.map((booking) => {
      if (booking.id === currentBooking.id) {
        ...
        let numberOfDays =
          newEndDate == null
            ? numberOfDaysOfBooking(currentBooking)
            : numberOfDaysBetweenDates(newStartDate, newEndDate);
        let allDatesForBooking = generateDates(newStartDate, numberOfDays);
        booking.from_date = formatDate(allDatesForBooking[0]);
        booking.to_date = formatDate(
          allDatesForBooking[allDatesForBooking.length - 1]
        );
      }
      return booking;
    });
