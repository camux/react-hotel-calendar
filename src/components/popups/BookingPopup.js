...
function BookingPopup(props) {
  ...
  const checkAvailabilityHandler = (e) => {
    let status = context.actionCanExistBooking(
      state,
      state.room_id,
      state.from_date,
      state.to_date
  ...
  const addGuestHandler = (e) => {
    if (
      ...
      newState.guests.push({
        ...
        security_number: document.querySelector("[name=new_guest_security_no]")
          .value,
      });
      setState(newState);
      document.querySelector("[name=new_guest_name]").value = "";
  ...
  if (state.checkAvailability) {
    bottomActionButton = (
      <button
        ...
        onClick={(e) => checkAvailabilityHandler(e)}
      >
        Check avalability
      </button>
    );
    ...
    bottomActionButton = (
      <button
        ...
        onClick={(e) => confirmBookingHandler(e)}
      >
        Confirm booking
      </button>
    );
        ...
        <div className="card">
          <div className="card-header text-center">
            <h5>
              <span>{heading}</span>
              <button
                onClick={() => context.actionClosePopup()}
                type="button"
                className="close"
                data-dismiss="modal"
                aria-label="Close"
          ...
          <div className="card-body">
            <form action="" className="form">
              ...
              <div className="text-right">
                <button
                  onClick={(e) => addGuestHandler(e)}
                  className="btn btn-success btn-block1"
                >
                  ADD GUEST
                </button>
              </div>
              ...
              <div className="text-center">{bottomActionButton}</div>
            </form>
          </div>
        </div>
      </div>
